...
- **3.3 Duo exploiter cycles** *(scaffolded only)*: config keys (`exploiter_mode`, `exploiter_cycle_steps`, `exploiter_priority_multiplier`) parse and validate, but the interleaved exploiter training is **deferred to a follow-up PR**. Opting in (`exploiter_mode='duo'`) prints a one-shot notice and logs `train/exploiter_cycle_deferred=1`. See ADR 0008.
