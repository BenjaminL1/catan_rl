...
## Vec env modes (parallelism)
- `vec_env_mode: 'serial' | 'subproc'` (default `'serial'`). Selected via the factory `make_vec_env` in `src/catan_rl/selfplay/vec_env_factory.py`. `'serial'` runs N envs in-process (legacy `GameManager`). `'subproc'` forks one subprocess per env via `multiprocessing.get_context("fork")`, communicating over `multiprocessing.Pipe`. Workers run `CatanEnv.step/reset/get_action_masks/get_opponent_obs_masks/apply_opponent_action`; opponent NN inference and league sampling stay main-process so the batched-opponent-inference perf optimization is preserved. **Empirical**: at `n_envs=8` on M1 Pro CPU-only, env stepping itself runs at ~5500 env-steps/sec in both modes — env stepping is *not* the training bottleneck, the policy forward pass at small batch is. Subproc speedup at the env-stepping layer is ~1.0-1.15× (effectively neutral). Code shipped for future use when batch-stepping becomes a bottleneck (larger n_envs, faster engine port, etc.); default keeps `'serial'` so all existing configs are unaffected.
