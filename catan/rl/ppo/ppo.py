...
from catan.rl.models.policy import CatanPolicy
from catan.rl.models.build_agent_model import build_agent_model
from catan.rl.models.utils import ValueFunctionNormalizer
from catan.rl.ppo.rollout_buffer import CompositeRolloutBuffer
from catan.rl.ppo.game_manager import GameManager
from catan.rl.ppo.evaluation_manager import EvaluationManager
from catan.rl.ppo.league import League
from catan.rl.ppo.utils import explained_variance, safe_mean
...
class CatanPPO:
    ...
    def __init__(self, config: Dict):
        ...
        model_kwargs = {
            k: config[k] for k in (
                "obs_output_dim", "value_hidden_dims", "action_head_hidden_dim",
                "tile_in_dim", "tile_model_dim", "curr_player_main_in_dim", "other_player_main_in_dim",
                "dev_card_embed_dim", "dev_card_model_dim", "tile_model_num_heads",
                "proj_dev_card_dim", "dev_card_model_num_heads", "tile_encoder_num_layers",
                "proj_tile_dim", "dropout",
            ) if k in config
        }
        ...
        self.use_entropy_annealing = (
            self._entropy_anneal_end > self._entropy_anneal_start
        )
    ...
    def update(self) -> Dict[str, float]:
        ...
        ev = explained_variance(
            self.buffer.values[:self.n_steps],
            self.buffer.returns[:self.n_steps]
        )
        ...
        if self.use_entropy_annealing:
            ...
            if start_u <= update_num <= end_u:
                ...
                self.entropy_coef = (
                    ...
                    + frac * (self._entropy_coef_final - self._entropy_coef_start)
                )
    ...
    def train(self, verbose: bool = True) -> None:
        ...
        try:
                ...
                if self.global_step - last_eval_step >= eval_interval:
                    ...
                    if len(window) >= self.stagnation_window:
                        ...
                        if improvement < self.stagnation_threshold:
                            print(f"  [WARN] STAGNATION detected: win rate range "
            ...
            self.writer.close()
            print(f"Training interrupted at step {self.global_step:,}. "
    ...
    @classmethod
    def load(cls, path: str, config_override: Optional[Dict] = None) -> "CatanPPO":
