...
from catan.rl.setup.setup_env import SetupEnv, OBS_DIM, N_EDGES, N_VERTICES
N_CORNERS = N_VERTICES  # alias: corner = vertex placement head size
from catan.rl.setup.setup_policy import SetupPolicy
from catan.rl.models.utils import ValueFunctionNormalizer
...
class SetupTrainer:
    ...
    def train(self, verbose: bool = True) -> None:
        ...
        print(f"SetupTrainer: training for {self.total_episodes:,} episodes "
        ...
        try:
            while self.global_episodes < self.total_episodes:
                ...
                if verbose:
                    print(f"Episode {self.global_episodes:>7,} | "
                ...
                self.writer.add_scalar("setup/pg_loss",        update_stats["pg_loss"],    self.global_episodes)
                self.writer.add_scalar("setup/value_loss",     update_stats["value_loss"], self.global_episodes)
                self.writer.add_scalar("setup/entropy",        update_stats["entropy"],    self.global_episodes)
                self.writer.add_scalar("setup/approx_kl",      update_stats["approx_kl"],  self.global_episodes)
    ...
    def _collect_rollouts(self) -> Dict:
        ...
        for env in self.envs:
            ...
            for _ in range(n_eps_per_env):
                ...
                while not done:
                    ...
                    cm_t    = torch.tensor(corner_mask_np, dtype=torch.bool, device=self.device).unsqueeze(0)
                    em_t    = torch.tensor(edge_mask_np,   dtype=torch.bool, device=self.device).unsqueeze(0)
                ...
                ep_returns = _compute_returns(
                    ep_rewards, gamma=self.gamma, gae_lambda=self.gae_lambda,
                    values=ep_values, last_value=0.0,
                )

                # Write to buffer
                for t_local, (ob, cm, em, ic, ac, rw, dn, vl, lp, ret) in enumerate(zip(
                    ep_obs, ep_corner_masks, ep_edge_masks, ep_is_corner,
                    ep_actions, ep_rewards, ep_dones, ep_values, ep_log_probs, ep_returns
                )):
                    if step_ptr >= n_steps:
                        break
                    obs_buf[step_ptr]         = ob
        ...
        for ep_start in range(0, n_valid, steps_per_ep):
            ...
            ep_rets  = _compute_returns(
                list(ep_rews), gamma=self.gamma, gae_lambda=self.gae_lambda,
                values=list(ep_vals), last_value=0.0,
            )
        ...
        self.value_normalizer.update(
            torch.tensor(returns_buf, dtype=torch.float32)
        )
    ...
    def _update(self, buffer: Dict) -> Dict:
        ...
        for _ in range(self.n_epochs):
            ...
            for start in range(0, n, self.batch_size):
                ...
                pg_loss2  = -adv_b * torch.clamp(ratio, 1.0 - self.clip_range,
                                                         1.0 + self.clip_range)
    ...
    @classmethod
    def load(cls, path: str, config_override: Optional[Dict] = None) -> "SetupTrainer":
