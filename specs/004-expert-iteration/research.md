...
NOT convert to a LARGE or measurable win-rate gain when played out.** Power caveat (audit
2026-06-17): n=30 states, per-state SD ~0.176 → this robustly rules out a large bankable
per-override gain but is UNDERPOWERED to exclude a small (~+2.5pp) positive (would need ~190
states). So the honest claim is "no large/measurable per-override gain," not "exactly zero." (Arc
reconciles: hard-label pilot flat (step 1) + tautological in-model margin (panel) + no measurable
out-of-model override gain (step 2).)

**FINAL VERDICT (audit-confirmed, conf 0.85):** ExIt policy-action distillation — hard OR soft, type
OR sub-action — is **NOT a strong lever for a stronger search-free policy on this architecture**.
Even the optimistic per-state CI ceiling (+0.088) diluted through the 7.3% override rate is a
negligible fast-policy WR shift, so the actionable conclusion holds regardless of the small-positive
residual. A senior-RL correctness audit (3 auditors + synthesis, live-reproduced the key numbers on
the real checkpoints) found STEP 1 and STEP 2 **correctly implemented** — CRN dice/opponent/agent
streams properly controlled, agent_outcome seat-correct, clean pre-noise prior captured, per-state
(not pooled) CI used; the +0.154 in-model margin is confirmed near-tautological and was correctly
quarantined to context (never gates the verdict). The course of action's Step-2 kill criterion fired
→ Step 4 (soft pipeline) NOT built. **Banked:** 003 inference search (+54.6 Elo, shipped, knobs
genuinely default-off → no regression) remains the deployed human-level lever.

**Capacity-check status (audit: the BC-policy-floor version is CONFOUNDED — do not ship it as a
plateau diagnostic).** The 5,055 labels were generated by sims=50 search from v6 (manifest
`base_ckpt=v6_1499`), so with the 7.3% override rate **~93% of policy-head labels ARE v6's own
argmax** → "fit the labels" ≈ "imitate v6," a function the v6-derived trunk represents by
construction. A flat 512-vs-1024 val-CE is consistent with BOTH "ample capacity" AND "target is
trivially v6," so the pre-registered "flatten ⇒ don't widen" rule is a non-sequitur (withdrawn).
Compounded by single-seed noise, a width-correlated minibatch-order confound (DataLoader shuffle
with no `generator=`), and a non-width-normalized budget — all inside the 0.02-nat band — at ~1
min/epoch×80×3 widths×≥3 seeds ≈ **~12 hrs of low-diagnosticity compute**. **The honest capacity
question lives IN the phenomenon:** a 512-vs-1024 SELF-PLAY run comparing anchor-WR trajectories,
funded as part of the self-play upgrade — not a BC proxy. **Recommended next (USER DECISION):** the
corrected SELF-PLAY restart (PFSP + frozen anchor + 1-2 exploiters, scaffolded-unwired) with a
512-vs-1024 width arm folded in; value-distill `best_q` stays a marginal cheap arm (sharpens the
search leaf only).
