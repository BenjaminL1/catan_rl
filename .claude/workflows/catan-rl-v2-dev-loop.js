...
// <<< veriloop:auto:start >>>
export const meta = {
  "name": "catan-rl-v2-dev-loop",
  "description": "Per-feature dev loop for catan-rl-v2: plan(vs constitution) -> triage -> worktree implement -> risk-tiered gate(real exit-code checks + review lenses) -> bounded auto-fix -> docs sync -> push preview. Stops before merge.",
  "phases": [
    {
      "title": "Plan",
...
const VERILOOP = {
  ...
  "depsSetup": "create/enter a virtualenv (or reuse the project's), then `python -m pip install -e \".[dev]\"` inside the worktree so imports + entry points resolve. This repo ships a compiled (Rust/maturin) extension — also run `make rust-build` in the worktree so the extension is importable.",
  "crossModel": true,
  "budget": {
    "posture": "balanced",
    "presets": {
