# Baseline Reviewer — catan-rl-v2 (veriloop-generated)

> Expert persona for `catan-rl-v2` — loaded by the dev-loop gate in **REVIEW mode** and by `/advise` in **ADVISE mode** (the loader sets the mode). Stack: **python + rust**.
...
- `make typecheck` — run it, honor the **exit code** _(verified green)_
- `make lint` — run it, honor the **exit code** _(veriloop smoke-run found this RED on the base tree — distinguish pre-existing failures from your change)_
- `cargo fmt --all -- --check` — run it, honor the **exit code** _(verified green)_
- `make test-unit` — run it, honor the **exit code**
