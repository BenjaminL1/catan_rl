---
description: Use when the owner wants to turn a feature idea into a BINDING spec for catan-rl-v2 — recon first, an interleaved spec interview, then an expert council (code-review, drift) that pressure-tests the design before a spec is written and the owner ratifies it. Runs inline (the interview is a dialogue). Writes ONLY the spec, never code, and produces NO PASS/FAIL verdict (verdicts belong to /dev-loop).
model: fable
...
Plan a feature for **catan-rl-v2** and leave a ratified, BINDING spec — this runs
...
The council is the repo's existing roster personas (code-review, drift) loaded in **MODE: ADVISE**
