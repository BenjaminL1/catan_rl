---
description: Use when the owner wants to brainstorm a feature or direction, sanity-check a design decision, weigh priorities, or pressure-test an idea BEFORE building — a consultation with catan-rl-v2's expert personas (code-review, drift) in ADVISE mode. The dialogue is inline; a MANDATORY read-only premise-council then pressure-tests the recommendation before it lands. Read-only; produces advice + tradeoffs, never a PASS/FAIL verdict (verdicts belong to /dev-loop).
allowed-tools: Read, Grep, Glob, AskUserQuestion, Task, WebSearch, WebFetch, Bash(git log:*), Bash(git diff:*), Bash(git show:*), Bash(make typecheck:*), Bash(make lint:*), Bash(cargo fmt --all -- --check:*), Bash(make test-unit:*)
---

Consult **catan-rl-v2's experts** on an idea — the DIALOGUE runs **inline, in the main
...
5. **Convene the premise-council — ALWAYS.** `/advise` guides direction, and the costliest
   ...
   - **Spawn each roster expert (code-review, drift) PLUS a dedicated PREMISE reviewer as parallel,
