export const meta = {
  name: 'human-data-stage2-build',
  description: 'STAGE-2 CONTINUATION: Stage 1 (ingest/logparse/segment) + board_cv are built+fixed on main; this builds ONLY the remaining modules (openings, validate, glyph_anchor, batch) via the same review-and-resolve loop, then Tier-5/gold gates + glyph-gated harvest + report. Scope-locked to src/catan_rl/human_data + scripts/mine_phantom.py; never rebuilds/overwrites the already-built modules.',
  phases: [
...
ALREADY BUILT + FIXED ON MAIN (reuse via import, NEVER rewrite/overwrite these files): ingest.py,
logparse.py (handle-aware chat firewall — a leading-handle ':' is chat), segment.py (merge-favouring
boundaries — ambiguous identical resets merge-then-discard, never guess a split), board_cv.py
(orientation-locked lattice). Your job is ONLY the remaining modules: openings.py, validate.py, the
glyph_anchor firewall, and batch.py + the scripts/mine_phantom.py CLI.
...
if (!scaffold.ready) {
  ...
  // Stage 1 (ingest, logparse, segment) is ALREADY BUILT + FIXED on main (this session:
  // logparse handle-aware chat firewall + segment merge-favouring boundary). Do NOT
  // rebuild it — the Stage-1 gate below exercises the existing modules end-to-end.
  ...
  // only the integration tiers below are gated on the ready subset.
  {
    ...
    // board_cv is ALREADY BUILT on main (orientation-locked lattice, hardened). Build
    // only the remaining Stage-2 modules on top of it.
