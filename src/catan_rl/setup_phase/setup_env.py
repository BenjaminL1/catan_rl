...
OBS_DIM = (
    N_TILES * TILE_FEAT_DIM + N_VERTICES * VERTEX_FEAT_DIM + N_EDGES * EDGE_FEAT_DIM + META_DIM
)  # = 1417
...
class SetupEnv(gym.Env):
    ...
    def __init__(self, n_rollouts: int = 20, max_game_turns: int = 500):
        ...
        self.agent_player: heuristicAIPlayer | None = None
        self.opponent_player: heuristicAIPlayer | None = None
        self._setup_step: int = 0  # 0-3

        # State saved for rollout replay
        self._rng_state: Any | None = None  # numpy RNG before board creation
        self._agent_decisions: list = []  # [(kind, pixel_or_edge), ...]
        # kind: 'settle' → (px,), 'road' → (v1_px, v2_px)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        valid_settle = board.get_setup_settlements(self.agent_player) if step in (0, 2) else {}
        ...
        for e_idx, (v1_px, v2_px) in enumerate(self._edge_list):
            ...
            is_valid = (
                1.0
                if ((v1_px, v2_px) in valid_roads_raw or (v2_px, v1_px) in valid_roads_raw)
                else 0.0
            )
            obs.append(is_valid)
